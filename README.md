...
│  - 解密 data 卷后切换 root                │
├─────────────────────────────────────────┤
│  Layer 1: Alibaba Cloud Intel TDX ECS   │
│  - TDX 内存加密引擎 (MEE)                 │
│  - 硬件级信任根                           │
└─────────────────────────────────────────┘
        │
...
最底层是阿里云 g8i （或g9i） 实例提供的 Intel TDX 可信执行环境，TDX 通过内存加密引擎（MEE）对整个 Guest OS 的内存进行透明加密。系统采用 UKI（Unified Kernel Image）启动方式，将 UEFI 引导程序、内核、initrd 打包为单个 EFI 可执行文件，由 `10-install-attestation.sh` 配置生成。initrd 中的 `cai-secret-fetch` 服务（由 `13-install-secret-supplicant.sh` 安装）在启动早期通过 TNG 连接 Trustee KBS，获取磁盘加密密钥后解密 data 卷，然后切换到 rootfs 继续启动。
...
1. **实例规格**: 必须使用支持 Intel TDX 的 g8i 或 g9i 实例规格
